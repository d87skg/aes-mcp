...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('aes.install', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('aes.configure', async () => {
            ...
            vscode.window.showInformationMessage(
                'AES MCP configured! Restart Cursor to activate. $(shield)'
            );
        })
    );
    ...
    // 命令：显示状态
    context.subscriptions.push(
        vscode.commands.registerCommand('aes.showStatus', () => {
            vscode.window.showInformationMessage(
                'AES Runtime Security v2.0.0 — Active $(shield)'
            );
        })
    );
    ...
    // 命令：运行 Demo
    context.subscriptions.push(
        vscode.commands.registerCommand('aes.demo', async () => {
    ...
    // 命令：打开 Trace Viewer
    context.subscriptions.push(
        vscode.commands.registerCommand('aes.traceViewer', async () => {
